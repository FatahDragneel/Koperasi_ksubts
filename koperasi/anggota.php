...
$sql = "SELECT a.*,
    ...
    (SELECT GROUP_CONCAT(DISTINCT CONCAT_WS(' · ', g.kode_gapoktan, g.nama_gapoktan) ORDER BY g.kode_gapoktan SEPARATOR ', ') FROM anggota_gapoktan ag JOIN gapoktan g ON g.id=ag.id_gapoktan WHERE ag.anggota_id=a.id) AS semua_gapoktan,
    (SELECT GROUP_CONCAT(DISTINCT CONCAT_WS(' · ', l.kode_koperasi, l.nama_koperasi) ORDER BY l.kode_koperasi SEPARATOR ', ') FROM anggota_lembaga al JOIN lembaga_koperasi l ON l.id=al.id_koperasi WHERE al.anggota_id=a.id) AS semua_koperasi
  FROM anggota a
    ...
    <?php foreach ($rows as $r): ?>
      <tr>
        ...
        <td>
          <?php
          $segLembaga = [];
          $kodeUtama  = (string)($r['kode_kelompok'] ?? '');
          $daftarKel  = array_values(array_filter(array_map('trim', explode(',', (string)($r['semua_kelompok'] ?? '')))));
          $kelLain    = array_values(array_filter($daftarKel, static function ($k) use ($kodeUtama) {
              return $k !== '' && $k !== $kodeUtama;
          }));
          if ($kodeUtama !== '') {
              $segLembaga[] = '<strong>'.e($kodeUtama).'</strong>'
                  . (!empty($r['nama_kelompok']) ? '<br><small>'.e($r['nama_kelompok']).'</small>' : '')
                  . (!empty($r['nama_ketua']) ? '<br><small>Ketua: '.e($r['nama_ketua']).'</small>' : '');
          }
          if ($kelLain) {
              $segLembaga[] = '<strong>'.e(implode(', ', $kelLain)).'</strong> <small>(kelompok)</small>';
          }
          if ($kodeUtama === '' && !$kelLain && !empty($r['plasma'])) {
              $segLembaga[] = label_kelompok($r['kelompok_tani']);
          }
          if (!empty($r['semua_gapoktan'])) {
              $segLembaga[] = '<strong>'.e($r['semua_gapoktan']).'</strong> <small>(gapoktan)</small>';
          }
          if (!empty($r['semua_koperasi'])) {
              $segLembaga[] = '<strong>'.e($r['semua_koperasi']).'</strong> <small>(koperasi)</small>';
          }
          echo $segLembaga ? implode('<br>', $segLembaga) : '—';
          ?>
        </td>
        <td>
